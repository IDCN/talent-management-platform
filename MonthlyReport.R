...
#Directories
main_dir <- file.path("", "Users","jon","IDCN","Monthly_Report")
date_dir <- toString(Sys.Date())
plot_dir <- "plots"
getReportDir <- file.path(date_dir)
getPlotDir <- file.path(date_dir, plot_dir)

#Set working directory.
setwd(main_dir)

#Create directories for the Monthly Report.
ifelse(!dir.exists(getReportDir), 
...
TMP_activations <- TMP_active[,c(1,2,44)]
TMP_deactivations <- TMP_inactive[,c(1,2,42,43)]
...
TMP_volunteers_inactive <- dplyr::filter(TMP_volunteers, TMP_volunteers[,4]=="Inactive")
TMP_volunteers_ex_active <- dplyr::filter(TMP_volunteers, TMP_volunteers[,4]=="Active" & TMP_volunteers[,8]!="0000-00-00")
...
ggplot(counts, aes(x = Company, y = count)) +
  ...
  geom_text(aes(label = count, y = count + 4), size = rel(7), family="Calibri") +
...
#VOLUNTEER PARTNERS
TMP_volunteers_alumni_employed <- dplyr::filter(TMP_inactive, TMP_inactive[,34]!="-")
TMP_volunteers_alumni_employed <- dplyr::filter(TMP_volunteers_alumni_employed, grepl('Job', TMP_volunteers_alumni_employed[,43]))
TMP_volunteers_alumni_employed_corporate <- dplyr::filter(TMP_volunteers_alumni_employed, grepl('Corporate', TMP_volunteers_alumni_employed[,43]))
TMP_volunteers_alumni_employed_other <- dplyr::filter(TMP_volunteers_alumni_employed, grepl('Other', TMP_volunteers_alumni_employed[,43]))

volunteers_active <- nrow(TMP_volunteers_active)
volunteers_inactive <- nrow(TMP_volunteers_inactive)
volunteers_ex_active <- nrow(TMP_volunteers_ex_active)
volunteers_alumni_employed <- nrow(TMP_volunteers_alumni_employed)
volunteers_alumni_employed_corporate <- nrow(TMP_volunteers_alumni_employed_corporate)
volunteers_alumni_employed_other <- nrow(TMP_volunteers_alumni_employed_other)
...
ggplot(Evolution, aes(x=Month, y=Count, group=1)) +
  ...
  geom_text(aes(label = Count, y = Count+50), size=rel(7), family="Calibri")
...
ggplot(CPEC, aes(x=Year, y=Count, group=1)) +
  ...
  geom_text(aes(label = Count, y = Count+1), size=rel(8), family="Calibri") +
...
ggplot(CPEO, aes(x=Year, y=Count, group=1)) +
  ...
  geom_text(aes(label = Count, y = Count+5), size=rel(8), family="Calibri") +
