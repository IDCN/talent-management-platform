...
#Returns the column titles, together with the column number.
#Useful for referencing columns without using string titles.
names(TMP)
B[,27]

#For display purposes, replace "-" values with "(No Response)".
B[,14][B[,14] == "-"] <- "(No Response)"
B[,18][B[,18] == "-"] <- "(No Response)"
B[,27][B[,27] == "-"] <- "(No Response)"
...
B1 <- data.frame(
  ...
  Last = B$"Last Name",
  Education = B[,27],
  Desired = B[,14], #Column 14 is "Desired Industry"
...
#or job role.
source("~/TMP-R/TDP/constructGraphs.R")